...
describe("Test unaryop layer of WebGPU backend", () => {
    it("Test identity operation", async () => {
        ...
        const output = await handleUnaryOp(input, "output[global_id.x] = input[global_id.x]", device!);
    ...
    it("Test abs operation", async () => {
        const device = await getGPUDevice();
        const input = TensorBuilder.withData([-1, 0, 1]);
        const output = await handleUnaryOp(input, "output[global_id.x] = abs(input[global_id.x])", device!);
