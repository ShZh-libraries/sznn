    ...
    (
    ...
    ) => {
        {
            ...
            (0..$in_shape[1]).into_par_iter().for_each(|c| {
                ...
                // Calculate variance
                let mut variance = 0.;
                $arr[offset..(offset + $channel_size)].into_iter().for_each(|item| {
                ...
                for i in 0..$channel_size {
                    ...
                    out[offset + i] = weight[c] * ($arr[offset + i] - mean) / (variance + $epsilon as $typ).sqrt() + bias[c];
                }
            });
...
pub fn handle_instancenorm(
    ...
    let out_data = match &input.get_data() {
        DTypes::F32(arr) => DTypes::F32(instancenorm!(arr, weight, bias, epsilon, in_shape, channel_size, f32, DTypes::F32)),
        DTypes::F64(arr) => DTypes::F64(instancenorm!(arr, weight, bias, epsilon, in_shape, channel_size, f64, DTypes::F64)),
