...
#[wasm_bindgen(js_name = handleSqrt)]
pub fn handle_sqrt(input: &Tensor) -> Tensor {
    ...
    let out_data = match &input.get_data() {
        ...
        DTypes::F64(arr) => {
            ...
            arr.par_chunks(2)
                ...
                .for_each(|(src, dst)| unsafe {
                    let src = v128_load(src.as_ptr() as *const v128);
                    let result = f64x2_sqrt(src);
                    v128_store(dst.as_mut_ptr() as *mut v128, result);
                });
            DTypes::F64(out)
        }
        _ => panic!("Data type not supported in sqrt layer!"),
    };
