...
export function forward(input: Tensor, weight: Tensor, bias: Tensor, epsilon: number): Tensor {
    ...
    for (let c = 0; c < output.shape[1]; c++) {
        ...
        for (let i = 0; i < channelSize; i++) {
            output.data[c * channelSize + i] = 
                weight.data[c] * (input.data[c * channelSize + i] - mean) / Math.sqrt(variance + epsilon) + bias.data[c];
        }
    }
