...
export function handleBinaryOp(inputs: Tensor[], op: BinaryFunc): Tensor[] {
    const a = inputs[0];
    const b = inputs[1];
    const output = forward(a, b, op);

    return [output];
}

export function forward(a: Tensor, b: Tensor, op: BinaryFunc): Tensor {
...
function getBroadcastShape(shape1: number[], shape2: number[]): number[] {
    let result = [];
    let resultLength = shape1.length > shape2.length? shape1.length : shape2.length;

    for (let index = 0; index < resultLength; index++) {
        let a = shape1[shape1.length - 1 - index]? shape1[shape1.length - 1 - index] : 1;
        let b = shape2[shape2.length - 1 - index]? shape2[shape2.length - 1 - index] : 1;
