...
export async function handleUpSample(input: Tensor, scales: Tensor, device: GPUDevice) {
    let outputShape = [];
    ...
    output.data = await computePass(resouces, [Math.ceil(output.getLength() / workgroup_size)], program, device, GPUDataEnum.Float32Array);
