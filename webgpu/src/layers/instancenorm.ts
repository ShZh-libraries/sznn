...
async function caclMean(input: Tensor, temp: Tensor, device: GPUDevice) {
    ...
    means.data = await computePass(resources, [Math.ceil(input.shape[1] / 8), Math.ceil(channelSize / 32)], program, device, GPUDataEnum.Float32Array);
...
async function calcVariance(input: Tensor, means: Tensor, temp: Tensor, device: GPUDevice) {
    ...
    variances.data = await computePass(resources, [Math.ceil(input.shape[1] / 8), Math.ceil(channelSize / 32)], program, device, GPUDataEnum.Float32Array);

    return variances;
}

async function forward(input: Tensor, weight: Tensor, bias: Tensor, means: Tensor, variances: Tensor, epsilon: number, device: GPUDevice) {
    ...
    output.data = await computePass(resources, [Math.ceil(len / 256)], program, device, GPUDataEnum.Float32Array);
